...
import { tableColumnHandler } from "pages/Nsi/utils";
import { StateType } from "types";
import { ColumnDefType, SchemeType } from "../api/params/nsi-page.params";
import { FieldSelectorsCrossItemsType, SchemeDataResponseType } from "../api/responses/nsi-page.response";
import { createNewRowInSchemeDataTC, deleteRowInSchemeDataTC, getAllSchemesTC, getDataSelectorsTC, getSchemeDataTC, updateRowInSchemeDataTC } from "../thunks/nsiPage";
...
const initialState = {
  ...
  columnDefs: [] as Array<ColumnDefType>,
  rowData: [] as any,
...
const nsiSlice = createSlice({
  ...
  reducers: {
    ...
    setColumnDefs: (state, action: PayloadAction<SchemeType>) => {
      state.columnDefs = tableColumnHandler(action.payload.fields)
    },
...
//exports
export default nsiSlice.reducer;
export const nsiActions = nsiSlice.actions;
export const nsi = (state: StateType): NsiStateType => state.nsi;

//types
export type NsiStateType = typeof initialState;
